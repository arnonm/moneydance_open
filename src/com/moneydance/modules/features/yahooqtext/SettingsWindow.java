...
import com.moneydance.modules.features.tasemaya.Maya;
  ...
  implements PropertyChangeListener
{
  ...
  private final JCheckBox _supportTLV = new JCheckBox();
...
//    setIconImage(Main.getIcon()); // available in Java 1.6 only
  ...
  private void initUI(final FeatureModuleContext context) {
    ...
    _supportTLV.setText(_resources.getString(L10NStockQuotes.SUPPORT_TASE));
    _supportTLV.setSelected(IsTLVSupported());
    fieldPanel.add(_supportTLV, GridC.getc(0, 6).colspan(5).field());

    statusSummaryPanel.setText(" ");
    fieldPanel.add(statusSummaryPanel, GridC.getc(0, 7).colspan(5).field());
    fieldPanel.setBorder(BorderFactory.createEmptyBorder(UiUtil.DLG_VGAP, UiUtil.DLG_HGAP,
                                                         0, UiUtil.DLG_HGAP));
  ...
  private boolean IsTLVSupported() {
    try {
      Class.forName("com.moneydance.modules.features.tasemaya.Maya", false, this.getClass().getClassLoader());
      System.err.println("Tase - support for TLV Exchange found");
      return true;
    } 
    catch (ClassNotFoundException e){
      System.err.println("Tase - support for TLV Exchange not found");
      return false;
    }
  }
...
//    _table.setFillsViewportHeight(true); // available in Java 1.6 only
  ...
  private void saveControlsToSettings() {
    ...
    prefs.setSetting(Main.SUPPORT_TLV, _supportTLV.isSelected());
